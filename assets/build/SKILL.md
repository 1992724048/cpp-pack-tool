...
| `stage_binaries(build_dir, out, config="Release", reset=False)` | 递归收集 `.lib/.a/.dll/.pdb`：Release → `release/lib/` + `release/bin/`；Debug → `debug/lib/` + `debug/bin/`（库类产物不落输出根；MinGW 的静态库 `lib*.a` 与导入库 `lib*.dll.a` 以 `.a` 结尾落 `lib/`）。跳过 CMake 中间目录；同名不同内容按父目录后缀去重（去重残留形如 `_build-*`）。`reset=True` 先递归删除本配置段再 staging，杜绝陈旧文件与去重改名跨构建累积；默认 `False` 保持「只增量补入」旧语义。 |
...
- **MinGW 要点**：产物为 GNU 命名（`lib*.dll` / `lib*.dll.a` / `lib*.a`；`stage_binaries` 按扩展名归类——`.dll` → `bin/`，`.a`（含导入库 `.dll.a`）→ `lib/`）；`windres.exe` 由工具从编译器同 bin 注入 `CMAKE_RC_COMPILER`（含 `.rc` 的工程无需自行配置）；不要套用 MSVC 旗标（`/O2`、`/MD` 等在 MinGW 下无效或报错），GNU 旗标由辅助模块统一注入；不要假设存在 vcvars 环境脚本（MinGW 工具链自包含，bin 已前置 PATH）。
