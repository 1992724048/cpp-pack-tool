...
| 设置 | 打包输出目录；编译器优先级（ICX/clang-cl/MSVC，检测本机版本、可排序与重新检测；检测结果缓存至 `detectedCompilers`，有缓存时免检测直接显示、「重新检测」刷新并写回）；SKILL.md 生成（内置模板写出，供分发给 AI 插件）；主题模式（系统/深色/浅色）、深色配色（Frappe/Macchiato/Mocha）与强调色，即时生效并持久化 |
