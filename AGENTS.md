...
- `ui/main_shell.dart` — 工作台 Scaffold：左栏库列表（含设置按钮+主源目录配置写入）+ 右栏四 Tab（包信息/文件映射/编译配置/打包）+ 底部日志面板；启动加载输出目录注册表（源目录配置优先覆盖）
- `ui/pages/` — 四个 Tab 页：pack_info（包元数据表单，README/LICENSE 自动填充默认值）/ file_mapping（映射表：源文件模式/目标 #include 路径/条件，行内编辑）/ build_config（C/C++ 标准、宏/附加目录/附加依赖均可增删列表、数据拷贝/源码注入）/ pack（打包模式切换 + 输出目录只读自全局设置 + 流式打包日志 + 文件丢失警告）
