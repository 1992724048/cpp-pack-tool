...
- 领域模型在 `lib/models/`：`PackModel`（含可选 `license`/`iconPath`/`sourcePath`，`toMap`/`fromMap` 序列化）、`CmdModel`/`CmdType`、`MacroModel`、`FileModel`/`FileType`（序列化只存 `path`/`size`，读回时由 basename 推导 name/type；类型识别覆盖 C++ 系与 pdb/asm/fortran/脚本/LLVM/Python/数据库等扩展）、`BuildModel`。
