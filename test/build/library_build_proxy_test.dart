...
// proxy 真机构建验收（P3 L2）：经生产链路 preparePackBuildEnvironment + runPackBuild
// 真实 clone https://github.com/microsoft/proxy.git，纯头文件库（无编译）：镜像
// include/proxy/**（顶层转发头 + v4 子目录）与源根许可证到 BUILD_OUT。
//
// 运行：$env:CNP_REAL_LIBRARY_BUILDS='proxy'; flutter test test/build/library_build_proxy_test.dart
...
const String _libraryName = 'proxy';
const String _recipePath = 'test/build/library_recipes/proxy/build.py';

/// gated 运行开关：`CNP_REAL_LIBRARY_BUILDS` 含 `proxy` 或 `all` 才执行。
String? _gateReason() {
  ...
  if (!selected.contains(_libraryName) && !selected.contains('all')) {
    return "设置 CNP_REAL_LIBRARY_BUILDS='proxy'（或 all）运行真实构建";
  }
  return null;
}
...
void main() {
  ...
  test(
    'proxy 真机构建：拉取源码 → 头文件/许可证分类入库',
    () async {
      final Stopwatch total = Stopwatch()..start();
      final Directory tempRoot = _createTempDir('cnp_lib_proxy_');
      ...
      final String mainHeaderPath = joinPath(includeDir, 'proxy/proxy.h');
      final String v4HeaderPath = joinPath(includeDir, 'proxy/v4/proxy.h');
      final String macrosHeaderPath = joinPath(
        includeDir,
        'proxy/proxy_macros.h',
      );
      final String licensePath = joinPath(packDir.path, 'LICENSE');
      final List<String> headers = _filesWithExtension(
        joinPath(includeDir, 'proxy'),
        '.h',
      );

      print(
        '[evidence] include/proxy/proxy.h=${File(mainHeaderPath).existsSync()}',
      );
      print(
        '[evidence] include/proxy/v4/proxy.h=${File(v4HeaderPath).existsSync()}',
      );
      print(
        '[evidence] include/proxy/proxy_macros.h='
        '${File(macrosHeaderPath).existsSync()}',
      );
      ...
      expect(
        ...
        isTrue,
        reason: 'include/proxy/proxy.h 主头应存在（顶层转发头）',
      );
      expect(
        File(v4HeaderPath).existsSync(),
        isTrue,
        reason: 'include/proxy/v4/proxy.h 应存在（主头转发目标，结构镜像）',
      );
      expect(
        File(macrosHeaderPath).existsSync(),
        isTrue,
        reason: 'include/proxy/proxy_macros.h 应存在',
      );
      expect(
        headers.length,
        greaterThanOrEqualTo(5),
        reason: 'proxy 头文件（含 v4 子目录）应全量镜像到 include/proxy',
      );
      expect(
        File(licensePath).existsSync(),
        isTrue,
        reason: 'proxy 源根 LICENSE 应经 stage_license 落 BUILD_OUT 根',
      );
      expect(
