...
#
# post.bat：在 BUILD_OUT 根生成（ANSI/ASCII 编码，cmd 原生可解析——UTF-8 无 BOM
# 的中文注释在 GBK 环境会被 cmd 解析出乱码命令行并报“不是内部或外部命令”），消费方
# 构建后以 `post.bat <目标可执行文件>` 调用，minject 以 --force --inplace 就地注入
# Release 的 mimalloc.dll；注入失败不阻断消费者构建（脚本恒 exit /b 0），兼容
# minject --help 用法（-f / -i / --postfix）。
#
...
POST_BAT_COMMENTS = (
    "由 cpp_nuget_pack 构建管线生成；系统脚本，请勿修改。",
    "将 mimalloc 注入消费者可执行文件（%~1 = 目标 exe 路径）；注入失败不阻断构建。",
)

# 本机 ANSI 代码页无法表示中文注释（如英文 Windows 的 cp1252）时的 ASCII 兜底。
POST_BAT_ASCII_COMMENTS = (
    "Generated by the cpp_nuget_pack build pipeline; system script, do not edit.",
    "Injects mimalloc into the consumer executable (%~1 = target exe path); never blocks the build.",
)
...
def post_bat_content(comments):
    """组装 post.bat 文本（CRLF 行尾；注释可替换以便按落盘编码退化）。"""
    lines = ["@echo off"]
    lines.extend("rem %s" % text for text in comments)
    lines.extend(
        (
            'if "%~1"=="" exit /b 0',
...
def write_post_bat():
    ...
    cmd 以系统 ANSI 代码页解析 .bat，UTF-8 无 BOM 写盘会解析出乱码命令行
    （实测报「不是内部或外部命令」）；优先按 mbcs/ANSI 落盘，代码页无法表示
    中文注释时退化为 ASCII 注释版本，保证 cmd 恒可解析。
    """
    path = os.path.join(BUILD_OUT, "post.bat")
    content = post_bat_content(POST_BAT_COMMENTS)
    try:
        data = content.encode("mbcs")
    except (LookupError, UnicodeEncodeError):
        data = post_bat_content(POST_BAT_ASCII_COMMENTS).encode("ascii")
